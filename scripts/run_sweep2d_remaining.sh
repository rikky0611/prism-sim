#!/usr/bin/env bash
# 2D sweep (REMAINING tasks): frame accuracy x communication cost.
#   accuracy axis: acc50 / acc75 / acc100  (frame recognizer accuracy 0.5/0.75/1.0)
#   comm-cost axis: c_nar=c_remind in {0.5, 1.58, 5.0}
#   tasks: the 5 not yet covered by run_sweep2d.sh (which did make_cereal, cooking)
...
#
# Usage:
#   nohup bash scripts/run_sweep2d_remaining.sh > data/logs/sweep2d_remaining_master.log 2>&1 &
...
TASKS=(latte_making make_coffee make_sandwich make_stencil make_tea)
