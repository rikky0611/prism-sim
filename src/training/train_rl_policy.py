...
def train_ppo_policy(
    ...
    print(f"  c_int={params.c_int}, c_fail={params.c_fail_base}, lambda={params.lambda_forget}")
...
def analyze_improvement(results: Dict[str, Dict[str, Any]]) -> None:
    ...
    if best_baseline_reward < 0:
        # Both negative (costs)
        improvement = (best_baseline_reward - rl_reward) / abs(best_baseline_reward) * 100
...
def plot_comparison(results: Dict[str, Dict[str, Any]], params: SimulationParams, save_path: str = "rl_comparison.png"):
    ...
    plt.suptitle(f'Policy Comparison (c_int={params.c_int}, c_fail={params.c_fail_base}, λ={params.lambda_forget})',
...
def main():
    ...
    # Test on balanced cost regime
    params = SimulationParams(
        c_int=5.0,  # Will be overridden by task defaults
        c_fail_base=12.0,  # Will be overridden by task defaults
        lambda_forget=0.05,
        f0_base=0.3,
        k_memory=2.0,
    )
    ...
    print(f"Cost regime: c_int={params.c_int}, c_fail={params.c_fail_base}")
    print(f"Cost ratio: c_fail/c_int = {params.c_fail_base/params.c_int:.2f}")
